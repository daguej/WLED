...
void handleTransitions() {
  ...
  if (transitionActive && strip.getTransition() > 0) {
    ...
    if (ti >= tr) {
      ...
      // segment transitions are not stopped here: each segment ends its own transition when its duration is over (Segment::handleTransition()),
      // as a segment may still be in a (longer) transition started before this global transition was restarted by another state change
      // AI: end
