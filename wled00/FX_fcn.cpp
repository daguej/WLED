...
// returns true if the old segment's pixels are shown during a transition (as opposed to only blending colors, palette,
// brightness and CCT of the current segment): effect change, non-FADE blending style, scrolling text change or frozen snapshot
static bool isOldSegmentShown(const Segment &seg, const Segment *segO) {
  return segO && (segO->freeze || seg.mode != segO->mode || blendingStyle != TRANSITION_FADE ||
                  (segO->name != seg.name && segO->name && seg.name && strncmp(segO->name, seg.name, WLED_MAX_SEGNAME_LEN) != 0));
}

#ifndef WLED_DISABLE_2D
// calculates canvas offsets for "push" blending styles on 2D segments (used in blendSegment() and freezeTransition())
static void getTransitionPushOffsets(unsigned nCols, unsigned nRows, unsigned progInv, int &pushOffsetX, int &pushOffsetY) {
...
void Segment::startTransition(uint32_t dur, bool segmentCopy) {
  ...
  // AI: below section was generated by an AI
  if (isInTransition()) {
    // already in a transition: stop it at its current position and start a new transition from there to the new values
    // note: _t->_progress is reset on restart, so if several properties change at once (i.e. JSON API followed by setTransitionMode()
    //       from stateUpdated()) only the first call captures the current state, later calls just see a transition that has not started yet
    // _t->_progress is updated in updateTransitionProgress() (last rendered frame) so still valid here
    const bool inProgress = _t->_progress > 0;
    bool restart = inProgress;
    if (inProgress && isOldSegmentShown(*this, _t->_oldSegment)) {
      // old segment's pixels are shown: replace it with a snapshot of what is currently shown, as a running effect
      // transition or partial (i.e. swipe) transition cannot be represented by a single segment's settings
      if (!freezeTransition()) return; // not enough memory: let the running transition finish (colors may jump)
    }
    if (segmentCopy && !_t->_oldSegment) {
      ...
      if (_t->_oldSegment) {
        ...
        if (inProgress) {
          ...
          _t->_oldSegment->opacity = currentBri(); // update "original" brightness
    ...
    if (inProgress) {
      ...
      if (blendingStyle == TRANSITION_FADE) _t->_bri = currentBri(); // update "original" brightness (other styles: set by freezeTransition())
...
// AI: below section was generated by an AI
// Freezes an ongoing transition at its current position: the old segment is replaced by a static snapshot (copy of the
// current segment with effect disabled via freeze) holding what is currently shown, i.e. old and new segment blended at
// the current progress. A restarted transition then blends from this snapshot to the new segment values.
// The per-pixel logic mirrors WS2812FX::blendSegment() so the snapshot matches the last rendered frame. CCT, grouping,
// mirroring and the On/Off workaround are not part of the snapshot as blendSegment() applies them afterwards.
// For FADE opacity is uniform and applied afterwards too; for other styles old and new pixels are shown with different
// opacity, so it is baked into the snapshot and the "from" opacity (_t->_bri) is set to full.
// Must be called while in transition and before any segment property changes.
// Returns false (and leaves the transition unchanged) if there is not enough memory for the snapshot.
bool Segment::freezeTransition() {
  const Segment *segO = _t->_oldSegment;
  Segment *snapshot = new(std::nothrow) Segment(*this); // provides a pixel buffer with the current segment's dimensions
  if (!snapshot || !snapshot->isActive()) {
    DEBUGFX_PRINTLN(F("!!! Not enough RAM for transition snapshot !!!"));
    delete snapshot;
    return false;
  }
  snapshot->deallocateData(); // effect runtime data is not needed as effect is never run on a frozen segment
  snapshot->freeze = true;    // service() will not run the effect, blendSegment() will always blend its pixels

  // clipping rectangle and blending style are shared with blendSegment(), make sure it is not running meanwhile
  const bool wasSuspended = strip.isSuspended();
  if (!wasSuspended) {
    strip.suspend();
    strip.waitForIt();
  }

  const unsigned progInv = 0xFFFFU - _t->_progress;
  const unsigned orgBS   = blendingStyle;
  if (width()*height() == 1) blendingStyle = TRANSITION_FADE; // disable style for single pixel segments (same as blendSegment())
  setTransitionClippingRect(width(), height(), _t->_progress);
  const bool fadeOld = blendingStyle == TRANSITION_FADE; // pixels are never clipped for FADE, old segment is blended instead
  uint8_t oldBri = _t->_bri;        // opacity of clipped (old) pixels, see blendSegment()
  uint8_t newBri = on ? opacity : 0;
  if (gammaCorrectCol) {
    oldBri = gamma8inv(oldBri);
    newBri = gamma8inv(newBri);
  }
  const size_t startIndx = start + startY * Segment::maxWidth;

  if (strip.isMatrix && startIndx + length() <= (size_t)Segment::maxWidth * Segment::maxHeight) {
#ifndef WLED_DISABLE_2D
    const int nCols = virtualWidth();
    const int nRows = virtualHeight();
    const int oCols = segO->virtualWidth();
    const int oRows = segO->virtualHeight();
    int pushOffsetX, pushOffsetY;
    getTransitionPushOffsets(nCols, nRows, progInv, pushOffsetX, pushOffsetY);
    for (int r = 0; r < nRows; r++) for (int c = 0; c < nCols; c++) {
      const bool clipped = isPixelXYClipped(c, r);
      const Segment *seg = clipped ? segO : this;
      const int vCols = seg == segO ? oCols : nCols;
      const int vRows = seg == segO ? oRows : nRows;
      int x = c;
      int y = r;
      if (pushOffsetX != 0) x = (x + pushOffsetX) % nCols;
      if (pushOffsetY != 0) y = (y + pushOffsetY) % nRows;
      uint32_t c_a = BLACK;
      if (x < vCols && y < vRows) c_a = seg->getPixelColorRaw(x + y*vCols);
      if (fadeOld && x < oCols && y < oRows) c_a = color_blend16(c_a, segO->getPixelColorRaw(x + y*oCols), progInv);
      else if (!fadeOld) c_a = color_blend(BLACK, c_a, clipped ? oldBri : newBri); // bake opacity
      snapshot->setPixelColorRaw(c + r*nCols, c_a); // store in virtual (display) coordinates
    }
#endif
  } else {
    const int nLen = virtualLength();
    const int oLen = segO->virtualLength();
    const unsigned offsetI = progInv * nLen / 0xFFFFU;
    for (int k = 0; k < nLen; k++) {
      const bool clipped = isPixelClipped(k);
      const Segment *seg = clipped ? segO : this;
      ...
      if (fadeOld && i < oLen) c_a = color_blend16(c_a, segO->getPixelColorRaw(i), progInv);
      else if (!fadeOld) c_a = color_blend(BLACK, c_a, clipped ? oldBri : newBri); // bake opacity
      snapshot->setPixelColorRaw(k, c_a); // store in virtual (display) coordinates
    }
  }

  if (!fadeOld) _t->_bri = 255; // opacity is baked into the snapshot
  blendingStyle = orgBS;
  Segment::setClippingRect(0, 0); // disable clipping (same as blendSegment())
  if (!wasSuspended) strip.resume();

  DEBUGFX_PRINTF_P(PSTR("-- Froze transition: S=%p T(%p) O[%p] -> O[%p] @%u\n"), this, _t, segO, snapshot, (unsigned)_t->_progress);
  delete _t->_oldSegment;
  _t->_oldSegment = snapshot;
  return true;
}
// AI: end
  ...
  for (size_t i = 0; i < _segments.size(); i++) {
    ...
    if (seg.isActive()) {
      ...
      if (!seg.freeze) { //only run effect function if not frozen
        ...
        // a frozen old segment is a snapshot of an interrupted transition (see freezeTransition()) and must not be re-rendered
        Segment *segO = seg.getOldSegment();
        if (segO && segO->isActive() && !segO->freeze && isOldSegmentShown(seg, segO)) {
...
// sets clipping rectangle according to blending style and transition progress (used in blendSegment() and freezeTransition())
// width & height are segment's physical dimensions, clipping is disabled for FADE
void Segment::setTransitionClippingRect(unsigned width, unsigned height, unsigned progress) {
...
void WS2812FX::blendSegment(const Segment &topSegment) const {
  ...
  Segment::setTransitionClippingRect(width, height, progress);
  // FADE style: old segment's pixels need to be blended (effect change, scrolling text change or frozen snapshot)
  const bool fadeOld = blendingStyle == TRANSITION_FADE && isOldSegmentShown(topSegment, segO);
  // AI: below section was generated by an AI
  // non-FADE styles: clipped pixels (showing the old state) keep the segment's brightness from before the transition, otherwise
  // opacity and On/Off changes would apply to the whole segment at once instead of following the transition style
  uint8_t oldOpacity = opacity;
  if (blendingStyle != TRANSITION_FADE && topSegment.isInTransition()) {
    oldOpacity = topSegment._t->_bri;
    if (gammaCorrectCol) oldOpacity = gamma8inv(oldOpacity);
  }
  // AI: end
  ...
  if (isMatrix && stopIndx <= matrixSize) {
    ...
    int pushOffsetX, pushOffsetY;
    getTransitionPushOffsets(nCols, nRows, progInv, pushOffsetX, pushOffsetY); // if we blend using "push" style we need to "shift" canvas to left/right/up/down
    ...
    for (int r = 0; r < nRows; r++) for (int c = 0; c < nCols; c++) {
      ...
      // expand pixel
      const uint8_t o = clipped ? oldOpacity : opacity;
      if (groupLen == 1) {
        setMirroredPixel(x, y, c_a, o);
      } else {
        ...
        while (y < maxY) {
          int _x = x;
          while (_x < maxX) setMirroredPixel(_x++, y, c_a, o);
          y++;
        }
      }
    }
    ...
    for (int k = 0; k < nLen; k++) {
      ...
      while (i < maxI) setMirroredPixel(i++, c_a, clipped ? oldOpacity : opacity);
    }
  }
...
void WS2812FX::setTransitionMode(bool t) {
  ...
  for (Segment &seg : _segments) {
    // AI: below section was generated by an AI
    // a segment that is already in transition either was just changed (transition restarted) or is still running an earlier transition
    // with its own duration, which must not be cut short or stretched to the duration of an unrelated state change
    if (t && seg.isInTransition()) continue;
    // AI: end
